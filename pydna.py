...
from keras.layers import Conv1D, LSTM, Dense, Dropout, Masking, Embedding, Flatten, MaxPooling1D
...
class PyDNA(object):
    ...
    @staticmethod
    def is_dna(dna_sequence_string, base_sequence=["A","C","G","T"], minimum_percentage=100):        
        total_dna_base = 0
        try:
            dna_sequence_string = dna_sequence_string.upper()
            for base in base_sequence:
                total_dna_base = total_dna_base + dna_sequence_string.count(base.upper())
    ...
    @staticmethod
    def dna_count_nucleotide(dna_sequence_string, base_sequence=["A","C","G","T"], is_length=None):
    ...
    @staticmethod
    def rna_count_nucleotide(rna_sequence_string, base_sequence=["A","C","G","U"], is_length=None):
    ...
    def dna_sequence_complement(dna_sequence_string):
        try:           
            complement_pairs = {"A":"T", "T":"A", "G":"C", "C":"G", "U":"A", "CT":"R", "AG":"Y", "GC":"S*", "AT":"W*", "T/UG":"M", "AC":"K", "CGT":"V", "AGT":"H", "ACT":"D", "ACG":"B", "ACGT":"N"}
            dna_complement_sequence = "".join([complement_pairs[base] for base in dna_sequence_string.upper()])          
        except:            
            print(PyDNA.get_exception_info())
            if PyDNA._app_is_log: PyDNA.write_log_file("error", PyDNA.get_exception_info())  
    ...
    def dna_sequence_reverse_complement(dna_sequence_string):
        try:           
            reverse_complement_pairs = {"A":"T", "T":"A", "G":"C", "C":"G", "U":"A", "CT":"R", "AG":"Y", "GC":"S*", "AT":"W*", "T/UG":"M", "AC":"K", "CGT":"V", "AGT":"H", "ACT":"D", "ACG":"B", "ACGT":"N"}
            dna_reverse_complement_sequence = "".join([reverse_complement_pairs.get(base, base) for base in reversed(dna_sequence_string.upper())])   
        except:            
            print(PyDNA.get_exception_info())
            if PyDNA._app_is_log: PyDNA.write_log_file("error", PyDNA.get_exception_info())  
    ...
    @staticmethod
    def is_rna(rna_sequence_string, base_sequence=["A","C","G","U"], minimum_percentage=100):        
        total_rna_base = 0
        try:
            rna_sequence_string = rna_sequence_string.upper()
            for base in base_sequence:
                total_rna_base = total_rna_base + rna_sequence_string.count(base.upper())
    ...
    @staticmethod
    def pca_explained_variance_plot(pca_n_components, explained_variance, min_explained_variance, font_size):       
        ...
        for rect in rects:    
            ...
            plt.text(rec_initial + rec_width / 2,  rec_height - min_explained_variance , str(float("{0:.3f}".format(rec_height))), horizontalalignment="center", verticalalignment="bottom", fontsize=font_size)              
    ...
    @staticmethod
    def pca_component_accuracy_scores_plot(pca_n_components, accuracy_score_valid, accuracy_score_test):
        font_size = 8
    ...
    @staticmethod
    def create_dataframe_column_words(dna_data_frame, input_column_name, output_column_name):  
        try:
            dna_data_frame[output_column_name] = dna_data_frame.apply(lambda x: PyDNA.k_mer_words_original(x[input_column_name]), axis=1)
    ...
    def dna_count_nucleotide(dna_sequence_string, is_length=None):
        ...
        try:
            ...
            print(PyDNA.get_exception_info())
            if PyDNA._app_is_log: PyDNA.write_log_file("error", PyDNA.get_exception_info())  
        return a_nucleotide, c_nucleotide, g_nucleotide, t_nucleotide, dna_sequence_length
    ...
    def rna_count_nucleotide(rna_sequence_string, is_length=None):
        ...
        try:
            ...
            print(PyDNA.get_exception_info())
            if PyDNA._app_is_log: PyDNA.write_log_file("error", PyDNA.get_exception_info())  
        return a_nucleotide, c_nucleotide, g_nucleotide, u_nucleotide, rna_sequence_length
...
# You may want to work with the reverse-complement of a sequence if it contains an ORF on the reverse strand. 
        ...
        try:
            if file_type == "CSV":
                if column_name is None:
                    ...
                    df_read = pd.read_csv(filepath_or_buffer=file_path_name, names=column_name)
            elif file_type == "TXT":
                if column_name is None:
                    df_read = pd.read_table(filepath_or_buffer=file_path_name)
                else:
                    df_read = pd.read_table(filepath_or_buffer=file_path_name, names=column_name)
            elif file_type == "JSON":
                pass
            elif file_type == "HTML":
                pass
    ...
    @staticmethod 
    def train_validation_test_split(X, y, is_stratify=True, test_size=0.2, valid_size=0.5, random_state_seed=50):
        try:
            if (is_stratify == True):
                X_train, X_test_valid, y_train, y_test_valid = train_test_split(X, y, test_size=test_size, stratify=y, random_state=random_state_seed)
                X_valid, X_test, y_valid, y_test = train_test_split(X_test_valid, y_test_valid, test_size=valid_size, stratify=y_test_valid, random_state=random_state_seed)
            else:
                X_train, X_test_valid, y_train, y_test_valid = train_test_split(X, y, test_size=test_size, random_state=random_state_seed)
                X_valid, X_test, y_valid, y_test = train_test_split(X_test_valid, y_test_valid, test_size=valid_size, random_state=random_state_seed)
        except:
            print(PyDNA.get_exception_info())
            if PyDNA._app_is_log: PyDNA.write_log_file("error", PyDNA.get_exception_info())  
    ...
    def create_ml_model(model_name, X_feature_train, y_label_train):
        try:
            if model_name == "MultinomialNB":
                ...
                ml_model = Pipeline([('Normalizing',MinMaxScaler()),('MultinomialNB',MultinomialNB())])
                ...
                ml_model = XGBClassifier(n_jobs=-1, random_state=50, objective="binary:softmax", num_class=7)
            elif model_name == "LogisticRegression":
                ml_model = LogisticRegression(multi_class="multinomial", solver ="newton-cg", n_jobs=-1, random_state=50)
                ...
                ml_model = LGBMClassifier(objective="multiclass", random_state=50, n_jobs=-1)
    ...
    @staticmethod
    def create_cnn_model(y_train, X_train, epochs_value, validation_split_value, val_accuracy_threshold, verbose_value=0):
        try:
            model = Sequential()
            model.add(Conv1D(filters=32, kernel_size=12, input_shape=(X_train.shape[1], 4)))
            model.add(MaxPooling1D(pool_size=4))
            model.add(Flatten())
            model.add(Dense(units=16, activation='relu'))
            model.add(Dense(units=2, activation='softmax'))
            model.compile(loss='binary_crossentropy', optimizer='adam', metrics=['accuracy'])
            callback = ValAccuracyCallback(val_accuracy_threshold=val_accuracy_threshold)
            history = model.fit(X_train, y_train, epochs=epochs_value, verbose=verbose_value, validation_split=validation_split_value, callbacks=[callback], use_multiprocessing=True)
            model.summary()
        except:
            print(PyDNA.get_exception_info())
            if PyDNA._app_is_log: PyDNA.write_log_file("error", PyDNA.get_exception_info())  
    ...
    @staticmethod
    def create_lstm_model(y_train, X_train, epochs_value, validation_split_value, val_accuracy_threshold, verbose_value=0):
        try:          
            model = Sequential()            
            model.add(LSTM(64, return_sequences=True, dropout=0.1, recurrent_dropout=0.1, input_shape=(X_train.shape[1], X_train.shape[2])))
            model.add(MaxPooling1D(pool_size=4))
            model.add(Flatten()) 
    ...
    def calculate_classification_metrics(y_original, y_predicted):   
        accuracy_score_value = precision_value = recall_value = f1_score_value = confusion_matrix_value = classification_report_value = None
        try:
            accuracy_score_value = float("{0:0.3f}".format( accuracy_score(y_original, y_predicted) * 100))    
            precision_value = float("{0:0.3f}".format( precision_score(y_original, y_predicted, average="weighted") * 100))
            recall_value = float("{0:0.3f}".format(recall_score(y_original, y_predicted, average="weighted") * 100))
            f1_score_value = float("{0:0.3f}".format(f1_score(y_original, y_predicted, average="weighted") * 100))
            ...
            print(PyDNA.get_exception_info())
            if PyDNA._app_is_log: PyDNA.write_log_file("error", PyDNA.get_exception_info())
        return accuracy_score_value, precision_value, recall_value, f1_score_value, confusion_matrix_value, classification_report_value
    ...
    @staticmethod
    def imbalanced_classes_rna_plot(ds_y, class_column_name, x_label, y_label, title, font_size=8):    
        try:        
            plt.figure(figsize = (5,5))
            sns.set(style="darkgrid")
            ax = sns.countplot(x=class_column_name, data=ds_y)
            ...
            for p in ax.patches:
                height = p.get_height()
                ax.text(p.get_x() + p.get_width()/2., height + 1, "{:1.0f}".format(height), ha="center", fontsize=font_size)     
            plt.title(title, fontsize=font_size)
            plt.show()
        except:
    ...
    def dna_sequence_is_equal_length(dna_sequence_series):
        try:
            dna_same_length = all(len(sequence)==len(dna_sequence_series[0]) for sequence in dna_sequence_series)
        except:
            print(PyDNA.get_exception_info())
            if PyDNA._app_is_log: PyDNA.write_log_file("error", PyDNA.get_exception_info())
    ...
    def cnn_X_onehot_encoder(X):        
        try:
            ...
            for x in X:
                label_encoder = LabelEncoder()  
                onehot_encoder = OneHotEncoder(sparse=False)
                x_onehot_encoded = PyDNA.dna_X_onehot_encoder(label_encoder, onehot_encoder, x)
    ...
    def dna_X_onehot_encoder(label_encoder, onehot_encoder, dna_sequence):
        try:
            ...
            dna_sequence_onehotencoder = onehot_encoder.fit_transform(dna_sequence_reshape)
        except:
            print(PyDNA.get_exception_info())
            if PyDNA._app_is_log: PyDNA.write_log_file("error", PyDNA.get_exception_info())
    ...
    @staticmethod
    def cnn_model_loss_plot(cnn_history, font_size, title_label, x_label, y_label, legend_label):
        try:
            plt.figure(figsize=[5,5])  
            plt.style.use('ggplot')
            plt.tick_params(labelsize=font_size)
    ...
    @staticmethod
    def cnn_model_accuracy_plot(cnn_history, font_size, title_label, x_label, y_label, legend_label):
        try:
            plt.figure(figsize=[5,5])  
            plt.style.use('ggplot')
            plt.tick_params(labelsize=font_size)
    ...
    @staticmethod
    def lstm_model_accuracy_plot(cnn_history, font_size, title_label, x_label, y_label, legend_label):
        try:
            plt.figure(figsize=[5,5])  
            plt.style.use('ggplot')
            plt.tick_params(labelsize=font_size)    
    ...
    def write_log_file(event_level, message):
        ...
        """       
        try:            
            logging_format_message = PyDNA.read_app_config_file("logging_format", "logging_format_message")
            logging_format_datetime = PyDNA.read_app_config_file("logging_format", "logging_format_datetime")
            if event_level == "CRITICAL".lower():
                logging.basicConfig(format=logging_format_message, datefmt=logging_format_datetime, 
                                    filename=PyDNA._app_log_file, level=logging.CRITICAL)  
                logging.critical(message)        
            elif event_level == "ERROR".lower():
                logging.basicConfig(format=logging_format_message, datefmt=logging_format_datetime, 
                                    filename=PyDNA._app_log_file, level=logging.ERROR)      
                logging.error(message)
            elif event_level == "WARNING".lower():
                logging.basicConfig(format=logging_format_message, datefmt=logging_format_datetime, 
                                    filename=PyDNA._app_log_file, level=logging.WARNING)      
                logging.warning(message)   
            elif event_level == "INFO".lower():
                logging.basicConfig(format=logging_format_message, datefmt=logging_format_datetime, 
                                    filename=PyDNA._app_log_file, level=logging.INFO)      
                logging.info(message)
            elif event_level == "DEBUG".lower():
                logging.basicConfig(format=logging_format_message, datefmt=logging_format_datetime, 
                                    filename=PyDNA._app_log_file, level=logging.DEBUG)      
    ...
    def get_exception_info(is_time_stamp=None):        
        ...
        try:     
            ...
            if is_time_stamp is not None:
                ...
                exception_info = ''.join('[File Name]: ' + str(file_name) + ' '
                + '[Procedure Name]: ' + str(procedure_name) + ' '
                + '[Error Message]: ' + str(exception_value) + ' '  
                + '[Error Type]: ' + str(exception_type) + ' '                                                                                                                                
                + '[Line Number]: ' + str(line_number) + ' '
    ...
    def is_string_empty_none(value_string):     
        ...
        """
        try:
            if(not (value_string and not value_string.isspace()) or (value_string is None)):
                is_empty_none = True
            else:
                is_empty_none = False
        except:
